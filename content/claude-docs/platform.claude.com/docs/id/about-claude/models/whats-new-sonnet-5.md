...
sha256: 51f0733ee9a3157eae89ca4ea9ee60558cabfecff3e42126044bd41b68511e85
---
...
Pada Claude Sonnet 4.6, permintaan tanpa field `thinking` berjalan tanpa thinking. Pada Claude Sonnet 5, permintaan yang sama berjalan dengan [adaptive thinking](/docs/id/build-with-claude/thinking). Untuk menonaktifkan thinking, kirimkan `thinking: {type: "disabled"}`. Karena `max_tokens` adalah batas keras untuk total output (thinking ditambah teks respons), tinjau kembali nilainya untuk beban kerja yang sebelumnya berjalan tanpa thinking di Claude Sonnet 4.6.

### Parameter sampling tidak diterima

Mengatur `temperature`, `top_p`, atau `top_k` ke nilai non-default mengembalikan error 400. Hapus parameter ini saat melakukan migrasi; nilai default (atau menghilangkan parameter tersebut) dapat diterima. Gunakan instruksi prompt sistem untuk mengarahkan perilaku model. Ini baru untuk model kelas Sonnet; batasan yang sama sebelumnya diperkenalkan pada Claude Opus 4.7.
...
* **Kapasitas jendela konteks dalam hal teks:** jendela konteks adalah 1M token, tetapi setiap token rata-rata mencakup lebih sedikit teks, sehingga jendela yang sama menampung lebih sedikit teks dibandingkan pada Claude Sonnet 4.6.
...
Peningkatan terbesar dibandingkan Claude Sonnet 4.6 adalah pada tugas coding dan agentik. Untuk hasil benchmark, lihat [Transparency Hub Anthropic](https://www.anthropic.com/transparency).
...
2. **Extended thinking:** jika Anda masih mengatur `budget_tokens`, migrasikan ke [adaptive thinking](/docs/id/build-with-claude/thinking). Extended thinking manual (`thinking: {type: "enabled"}`) tidak didukung dan mengembalikan error 400.
3. **Parameter sampling:** permintaan yang mengatur parameter sampling (`temperature`, `top_p`, `top_k`) ke nilai non-default mengembalikan error 400; hapus parameter tersebut saat melakukan migrasi. Definisi alat dan bentuk respons tidak berubah, dan prefilling pesan assistant memang sudah tidak didukung pada Claude Sonnet 4.6.
...
<CardGroup>
  ...
  <Card title="Adaptive thinking" icon="brain" href="/docs/id/build-with-claude/thinking">

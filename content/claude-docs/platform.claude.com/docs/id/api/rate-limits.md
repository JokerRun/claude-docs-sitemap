...
sha256: 710ed936d4a80f12da5cf9cba705610d2b188b143d6ef70dbe141c236f3557ae
---

---
title: Batas laju
url: https://platform.claude.com/docs/id/api/rate-limits
description: Untuk memitigasi penyalahgunaan dan mengelola kapasitas pada API, terdapat batasan mengenai seberapa banyak suatu organisasi dapat menggunakan Claude API.
---
...
1. **Batas pengeluaran** menetapkan biaya bulanan maksimum yang dapat ditanggung suatu organisasi untuk penggunaan API.
2. **Batas laju** ("rate limits") menetapkan jumlah maksimum permintaan API yang dapat dibuat suatu organisasi dalam periode waktu tertentu.

API memberlakukan batas yang dikonfigurasi layanan di tingkat organisasi, tetapi Anda juga dapat menetapkan batas yang dapat dikonfigurasi pengguna untuk workspace organisasi Anda.

## Tentang batas laju

* Batas dirancang untuk mencegah penyalahgunaan API, sekaligus meminimalkan dampak pada pola penggunaan pelanggan yang umum.
* Batas ditentukan berdasarkan **tingkat penggunaan** (usage tier). Organisasi ditempatkan pada suatu tingkat secara otomatis berdasarkan riwayat penggunaan dan status akun, serta dapat naik ke tingkat yang lebih tinggi seiring waktu saat mereka menggunakan API.
...
<Note>
  **[Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws):** Batas pengeluaran bekerja secara berbeda di Claude Platform on AWS. Lihat [Batas pengeluaran di Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws#spend-limits) untuk mengetahui bagaimana plafon pengeluaran dan batas pengeluaran yang Anda tetapkan sendiri berlaku untuk organisasi Anda.
</Note>

Masing-masing tingkat Start, Build, dan Scale memiliki plafon pengeluaran bulanan, yaitu jumlah maksimum yang dapat dibelanjakan organisasi Anda pada API setiap bulan kalender. Setelah Anda mencapai plafon pengeluaran tingkat Anda, penggunaan API dijeda hingga bulan berikutnya kecuali Anda meminta batas yang lebih tinggi. Anda dapat melihat plafon pengeluaran bulanan organisasi Anda dan menetapkan batas Anda sendiri di halaman [Billing](https://platform.claude.com/settings/billing).

| Tingkat penggunaan | Plafon pengeluaran bulanan |
| ------------------ | -------------------------- |
| Start              | $500 USD                   |
| Build              | $1.000 USD                 |
| Scale              | $200.000 USD               |

Organisasi di tingkat Custom tidak memiliki plafon pengeluaran bulanan; batasnya diatur bersama tim akun mereka.

Anda juga dapat menetapkan batas pengeluaran Anda sendiri di bawah plafon tingkat Anda untuk mengendalikan biaya:
...
<Steps>
  ...
  <Step title="Sesuaikan batas pengeluaran Anda">
    Masukkan nilai baru. Batas pengeluaran Anda tidak dapat melebihi plafon tingkat Anda saat ini.
  </Step>
</Steps>
...
Batas laju ITPM diestimasi di awal setiap permintaan, dan estimasi tersebut disesuaikan selama permintaan berlangsung untuk mencerminkan jumlah token input yang sebenarnya digunakan.
...
<Note>
  ...
  Ini berarti ketika Anda memiliki konten yang di-cache, `input_tokens` biasanya akan jauh lebih kecil daripada total input Anda. Misalnya, dengan dokumen yang di-cache sebesar 200 ribu token dan pertanyaan pengguna sebesar 50 token, Anda akan melihat `input_tokens: 50` meskipun total inputnya adalah 200.050 token.
...
**Contoh:** Dengan batas 2.000.000 ITPM dan tingkat cache hit 80%, Anda secara efektif dapat memproses 10.000.000 total token input per menit (2 juta tidak di-cache + 8 juta di-cache), karena token yang di-cache tidak dihitung terhadap batas laju Anda.
...
Batas laju OTPM dievaluasi secara real time saat token output dihasilkan, dengan hanya menghitung token yang benar-benar dihasilkan. Parameter `max_tokens` tidak diperhitungkan dalam kalkulasi batas laju OTPM, sehingga tidak ada kerugian batas laju dalam menetapkan nilai `max_tokens` yang lebih tinggi.
...
<Note>
  Batas laju saat ini dibagi bersama di semua nilai `inference_geo`. Permintaan dengan `inference_geo: "us"` dan `inference_geo: "global"` mengambil dari kumpulan batas laju yang sama.
</Note>
...
Selain menyediakan grafik token dan permintaan, halaman Usage menyediakan dua grafik batas laju terpisah. Gunakan grafik ini untuk melihat ruang yang Anda miliki untuk berkembang, mengidentifikasi kapan Anda mungkin mencapai penggunaan puncak, memahami batas laju apa yang perlu diminta, dan mempelajari cara meningkatkan tingkat caching Anda. Grafik ini memvisualisasikan sejumlah metrik untuk batas laju tertentu (misalnya, per model):
...
Untuk meminta batas laju yang lebih tinggi atau plafon pengeluaran bulanan yang lebih tinggi, gunakan **Request rate limit increase** di halaman [Rate limits](https://platform.claude.com/settings/limits). Dukungan Anthropic juga dapat menaikkan batas; untuk kebutuhan mendesak, hubungi [dukungan Anthropic](https://support.claude.com).

<Note>
  **[Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws):** Alur **Request rate limit increase** tidak tersedia. Hubungi perwakilan akun Anthropic Anda atau [dukungan Anthropic](https://support.claude.com), dan sertakan model yang perlu dinaikkan batasnya, puncak token input dan output per menit Anda untuk setiap model, serta kira-kira berapa bagian dari input Anda yang di-cache atau merupakan konteks berulang. Lihat [Batas laju dan kuota di Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws#rate-limits-and-quotas).
</Note>
...
| Header                                        | Deskripsi                                                                                                                             |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `retry-after`                                 | Jumlah detik yang harus ditunggu hingga Anda dapat mencoba ulang permintaan. Percobaan ulang yang lebih awal akan gagal.              |

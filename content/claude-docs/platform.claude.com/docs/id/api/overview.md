...
sha256: 54ea571e11a92c8f7cb901728d3444a65a0926862013182d6afdaeb8292825fb
---
...
<Note>
  **Baru mengenal Claude?** Untuk akses model langsung, mulailah dengan [Memulai](https://platform.claude.com/docs/id/get-started) dan [Bekerja dengan Messages](https://platform.claude.com/docs/id/build-with-claude/working-with-messages). Untuk infrastruktur agen terkelola, lihat [Panduan cepat Claude Managed Agents](https://platform.claude.com/docs/id/managed-agents/quickstart).
</Note>
...
* [Kunci API](https://platform.claude.com/settings/keys), atau aturan [Workload Identity Federation](https://platform.claude.com/docs/id/manage-claude/workload-identity-federation) yang telah dikonfigurasi

Untuk petunjuk penyiapan langkah demi langkah, lihat [Memulai](https://platform.claude.com/docs/id/get-started).
...
* **[Token Counting API](https://platform.claude.com/docs/id/api/messages-count-tokens)**: Hitung token dalam pesan sebelum mengirim untuk mengelola biaya dan batas laju (`POST /v1/messages/count_tokens`)
...
* **[Files API](https://platform.claude.com/docs/id/api/beta/files/upload)**: Unggah dan kelola file untuk digunakan di beberapa panggilan API (`POST /v1/files`, `GET /v1/files`)
* **[Skills API](https://platform.claude.com/docs/id/api/skills/create-skill)**: Buat dan kelola keterampilan agen kustom (`POST /v1/skills`, `GET /v1/skills`)
* **[Agents API](https://platform.claude.com/docs/id/managed-agents/agent-setup)**: Definisikan konfigurasi agen yang dapat digunakan kembali dan berversi untuk Claude Managed Agents (`POST /v1/agents`, `GET /v1/agents`)
...
Untuk referensi API lengkap dengan semua endpoint, parameter, dan skema respons, jelajahi halaman referensi API yang tercantum di navigasi. Untuk mengakses fitur beta, lihat [Header beta](https://platform.claude.com/docs/id/api/beta-headers).

## Autentikasi

Untuk detail tentang kedua metode autentikasi dan kapan menggunakan masing-masing, lihat [Autentikasi](https://platform.claude.com/docs/id/manage-claude/authentication). Semua permintaan ke Claude API harus menyertakan header berikut:
...
Jika Anda menggunakan [SDK Klien](https://platform.claude.com/docs/id/api/overview#client-sdks), SDK akan mengirim header ini secara otomatis. Untuk detail versi API, lihat [Versi API](https://platform.claude.com/docs/id/api/versioning).
...
API tersedia melalui [Console](https://platform.claude.com) web. Anda dapat menggunakan [Workbench](https://platform.claude.com/playground) untuk mencoba API di browser, lalu membuat kunci API di [Pengaturan Akun](https://platform.claude.com/settings/keys). Anda memilih [masa berlaku](https://platform.claude.com/docs/id/manage-claude/authentication#key-expiration) setiap kunci saat membuatnya. Gunakan [workspace](https://platform.claude.com/settings/workspaces) untuk mengelompokkan kunci API Anda dan [mengontrol pengeluaran](https://platform.claude.com/docs/id/api/rate-limits) berdasarkan kasus penggunaan.
...
| ---------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Agent Platform         | Google Cloud                             | [Claude di Google Cloud](https://platform.claude.com/docs/id/build-with-claude/claude-on-vertex-ai)              |
| Amazon Bedrock         | AWS                                      | [Claude di Amazon Bedrock](https://platform.claude.com/docs/id/build-with-claude/claude-in-amazon-bedrock)       |
| Claude Platform on AWS | AWS (dioperasikan Anthropic)             | [Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws)           |
| Microsoft Foundry      | Microsoft Azure (dioperasikan Anthropic) | [Claude di Microsoft Foundry](https://platform.claude.com/docs/id/build-with-claude/claude-in-microsoft-foundry) |

<Note>
  Claude Managed Agents tersedia melalui Claude API langsung dan [Claude Platform on AWS](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws). Untuk ketersediaan fitur di seluruh platform, lihat [Ikhtisar fitur](https://platform.claude.com/docs/id/build-with-claude/overview).
</Note>
...
| [Message Batches API](https://platform.claude.com/docs/id/build-with-claude/batch-processing) | 256 MB                     |
...
<Note>
  Platform yang dioperasikan mitra memiliki batas ukuran permintaan mereka sendiri: Bedrock membatasi permintaan hingga 20 MB, dan Google Cloud membatasi permintaan hingga 30 MB. Claude Platform on AWS menggunakan batas yang sama dengan Claude API langsung. Lihat dokumentasi platform Anda untuk nilai terkini.
</Note>
...
<Note>
  Claude Platform on AWS menambahkan ID permintaan AWS (`x-amzn-requestid`) di samping header `request-id` standar. Lihat [ID Permintaan](https://platform.claude.com/docs/id/build-with-claude/claude-platform-on-aws#request-ids) untuk pola penanganan ID ganda.
</Note>

## Paginasi

Endpoint daftar mengembalikan hasil dalam halaman. Sebagian besar endpoint daftar yang lebih baru menggunakan skema kursor `page` dan `next_page` yang dijelaskan di bagian ini. Beberapa menggunakan skema yang berbeda; lihat catatan di akhir bagian ini. Gunakan parameter kueri `limit` untuk mengontrol ukuran halaman dan parameter kueri `page` untuk mengambil halaman yang berdekatan. Setiap respons menyertakan array `data` bersama dengan field kursor untuk menavigasi antar halaman.
...
API menerapkan batas laju dan batas pengeluaran untuk mencegah penyalahgunaan dan mengelola kapasitas. Batas diatur ke dalam tingkatan penggunaan; organisasi Anda ditempatkan pada suatu tingkatan secara otomatis dan dapat berpindah ke tingkatan yang lebih tinggi seiring waktu. Setiap tingkatan memiliki:
...
Claude API tersedia di [banyak negara dan wilayah](https://platform.claude.com/docs/id/api/supported-regions) di seluruh dunia. Periksa halaman wilayah yang didukung untuk mengonfirmasi ketersediaan di lokasi Anda.

## Langkah selanjutnya

<CardGroup cols={2}>
  <Card title="Referensi Messages API" icon="book" href="https://platform.claude.com/docs/id/api/messages/create">
    Spesifikasi API lengkap untuk interaksi model langsung
  </Card>

  <Card title="Referensi Claude Managed Agents" icon="brain" href="https://platform.claude.com/docs/id/managed-agents/sessions">

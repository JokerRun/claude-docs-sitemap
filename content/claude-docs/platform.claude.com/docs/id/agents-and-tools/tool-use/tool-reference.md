...
sha256: b13c4a9ea4c2aa1abed91cae04ee92b6a86261140fa3faded641f631835681b6
---
...
Anthropic menyediakan dua jenis alat: **alat server** yang dieksekusi di infrastruktur Anthropic, dan **alat klien** di mana Anthropic mendefinisikan skemanya tetapi aplikasi Anda yang menangani eksekusinya. Kedua jenis alat ini muncul dalam array `tools` pada permintaan Anda bersama alat apa pun yang didefinisikan pengguna.
...
| [Alat web fetch](https://platform.claude.com/docs/id/agents-and-tools/tool-use/web-fetch-tool)              | `web_fetch_20260318` `web_fetch_20260309` `web_fetch_20260209` `web_fetch_20250910` | Server   | Tidak ada                                                           |
...
* **Berbasis kapabilitas:** `web_search_20260209` dan `web_fetch_20260209` menambahkan pemfilteran konten dinamis dibandingkan pendahulunya; `web_fetch_20260309` menambahkan opsi bypass cache; `web_search_20260318` dan `web_fetch_20260318` menambahkan kontrol penyertaan respons. `code_execution_20260120` menambahkan [pemanggilan alat terprogram](https://platform.claude.com/docs/id/agents-and-tools/tool-use/programmatic-tool-calling) dari dalam sandbox; `code_execution_20260521` mengungkapkan batas waktu per sel dalam deskripsi alat. Dalam setiap kasus, baik versi baru maupun lama sama-sama berlaku; versi mana yang Anda gunakan bergantung pada apakah Anda memerlukan kapabilitas baru tersebut.
* **Berbasis model:** `text_editor_20250728` ditujukan untuk model Claude 4 dan yang lebih baru, sedangkan `text_editor_20250124` untuk model sebelumnya. Versi yang Anda gunakan bergantung pada model yang Anda targetkan.
...
Tipe `mcp_toolset` tidak diberi versi berdasarkan tanggal; pemberian versinya dilakukan melalui header `anthropic-beta`.
...
Toolset klien adalah alat Messages API. Saat ini toolset klien tidak tersedia sebagai alat agen di [Claude Managed Agents](https://platform.claude.com/docs/id/managed-agents/tools), yang menyediakan toolset agen bawaan, toolset MCP, dan alat kustomnya sendiri.
...
`configs` menyesuaikan masing-masing anggota:
...
* Nama anggota yang tidak dikenal atau field lain apa pun dalam nilai anggota akan ditolak, begitu pula `configs` yang menonaktifkan setiap anggota (hilangkan entrinya saja).
...
* Dua entri dari toolset yang sama, atau alat lain yang menggunakan nama toolset tersebut: alat bernama `computer` bersama `computer_toolset_20260801`, atau alat bernama `browser` bersama `browser_toolset_20260801`. Kedua toolset dapat dideklarasikan bersamaan.

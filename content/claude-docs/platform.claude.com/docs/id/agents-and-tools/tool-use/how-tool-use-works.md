...
Sumbu utama yang membedakan alat adalah di mana kode dieksekusi. Setiap alat masuk ke dalam salah satu dari tiga kategori, dan kategori tersebut menentukan apa yang menjadi tanggung jawab aplikasi Anda.
...
Bentuk kanoniknya adalah loop `while` yang dikunci pada `stop_reason`:
...
* **Output terstruktur dengan bentuk yang terjamin.** Ketika Anda memerlukan objek JSON dengan field tertentu alih-alih prosa yang kebetulan berisi informasi tersebut, skema alat memaksakan bentuknya.
...
Tanda yang jelas bahwa Anda seharusnya menggunakan alat: jika Anda menulis regex untuk mengekstrak keputusan dari output model, keputusan tersebut seharusnya menjadi panggilan alat. Mem-parsing teks bebas untuk memulihkan maksud terstruktur adalah tanda bahwa struktur tersebut seharusnya berada di dalam skema.
...
* Interaksinya adalah tanya jawab satu kali tanpa efek samping. Jika tidak ada yang perlu dieksekusi, tidak ada yang perlu dilakukan oleh alat.

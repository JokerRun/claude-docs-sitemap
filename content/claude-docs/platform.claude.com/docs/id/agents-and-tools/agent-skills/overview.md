...
sha256: 129603e48b449e2972ea20a0ca074207222c5b31b90df91724d1ea773ff63e4c
---
...
* **Komposisikan kemampuan**: Gabungkan Skills untuk membangun alur kerja yang kompleks
...
Anthropic menyediakan Agent Skills bawaan untuk tugas dokumen umum (PowerPoint, Excel, Word, PDF), dan Anda dapat membuat Skills kustom Anda sendiri. Keduanya bekerja dengan cara yang sama. Claude secara otomatis menggunakannya saat relevan dengan permintaan Anda.
...
Claude memuat metadata ini saat startup dan menyertakannya dalam prompt sistem. Pendekatan ringan ini berarti Anda dapat menginstal banyak Skills tanpa penalti konteks; Claude hanya tahu bahwa setiap Skill ada dan kapan menggunakannya.
...
Saat Anda meminta sesuatu yang cocok dengan deskripsi sebuah Skill, Claude membaca SKILL.md dari filesystem melalui bash. Baru pada saat itulah konten ini masuk ke jendela konteks.

### Tingkat 3: Sumber daya dan kode (dimuat sesuai kebutuhan)

**Jenis konten: Instruksi, kode, dan sumber daya**. Skills dapat membundel materi tambahan:
...
**Kode**: Skrip yang dapat dieksekusi (fill\_form.py, validate.py) yang dijalankan Claude melalui bash; skrip menyediakan operasi deterministik tanpa mengonsumsi konteks
...
| **Tingkat 2: Instruksi**    | Saat Skill dipicu     | Di bawah 5k token             | Badan SKILL.md dengan instruksi dan panduan                                     |
| **Tingkat 3+: Sumber daya** | Sesuai kebutuhan      | Secara efektif tidak terbatas | File yang dibundel dieksekusi melalui bash tanpa memuat isinya ke dalam konteks |
...
**Akses file sesuai permintaan**: Claude hanya membaca file yang diperlukan untuk setiap tugas tertentu. Sebuah Skill dapat menyertakan puluhan file referensi, tetapi jika tugas Anda hanya memerlukan skema penjualan, Claude hanya memuat satu file itu. Sisanya tetap berada di filesystem tanpa mengonsumsi token sama sekali.
...
Model berbasis filesystem inilah yang membuat progressive disclosure bekerja. Claude menavigasi Skill Anda seperti Anda mereferensikan bagian tertentu dari panduan onboarding, mengakses tepat apa yang dibutuhkan setiap tugas.
...
2. **Permintaan pengguna**: "Ekstrak teks dari PDF ini dan ringkas"
...
![Skills dimuat ke dalam context window (jendela konteks) - menunjukkan pemuatan progresif metadata dan konten skill](/docs/images/agent-skills-context-window.png)

Diagram menunjukkan:

1. Keadaan default dengan prompt sistem dan metadata skill yang sudah dimuat sebelumnya
...
**Prasyarat:** Menggunakan Skills melalui API memerlukan [alat eksekusi kode](/docs/id/agents-and-tools/tool-use/code-execution-tool), yang container-nya menjadi tempat Skills berjalan, dan dua header beta:
...
Gunakan Agent Skills bawaan dengan mereferensikan `skill_id`-nya (misalnya, `pptx`, `xlsx`), atau buat dan unggah milik Anda sendiri melalui Skills API (endpoint `/v1/skills`). Skills kustom dibagikan di seluruh workspace; semua anggota workspace dapat mengaksesnya.

Untuk mempelajari lebih lanjut, lihat [Menggunakan Skills dengan Claude API](/docs/id/build-with-claude/skills-guide).

### Claude Code

[Claude Code](https://code.claude.com/docs/id/overview) hanya mendukung Skills Kustom.
...
**Skills Kustom**: Unggah Skills Anda sendiri sebagai file zip melalui Settings > Features. Tersedia pada paket Pro, Max, Team, dan Enterprise dengan eksekusi kode diaktifkan. Skills kustom bersifat individual untuk setiap pengguna; tidak dibagikan ke seluruh organisasi dan tidak dapat dikelola secara terpusat oleh admin.
...
**Field yang wajib**: `name` dan `description`
...
Gunakan Skills hanya dari sumber tepercaya: yang Anda buat sendiri atau yang diperoleh dari Anthropic. Skills memberikan Claude kemampuan baru melalui instruksi dan kode, dan meskipun ini membuatnya kuat, ini juga berarti Skill yang berbahaya dapat mengarahkan Claude untuk memanggil alat atau mengeksekusi kode dengan cara yang tidak sesuai dengan tujuan yang dinyatakan Skill tersebut.
...
* **Sumber eksternal berisiko**: Skills yang mengambil data dari URL eksternal menimbulkan risiko khusus, karena konten yang diambil mungkin berisi instruksi berbahaya. Bahkan Skills yang dapat dipercaya dapat dikompromikan jika dependensi eksternalnya berubah seiring waktu
* **Penyalahgunaan alat**: Skills yang berbahaya dapat memanggil alat (operasi file, perintah bash, eksekusi kode) dengan cara yang merugikan
...
* **[Claude API](/docs/id/agents-and-tools/agent-skills/claude-api-skill)**: Memberikan Claude materi referensi API terkini, dokumentasi SDK, dan praktik terbaik untuk 8 bahasa pemrograman. Dibundel dengan Claude Code dan juga tersedia untuk instalasi dari repositori skills.
...
Memahami batasan ini membantu Anda merencanakan deployment Skills Anda secara efektif. Claude Platform on AWS dan Microsoft Foundry mengikuti batasan yang sama dengan Claude API di subbagian berikut.

### Ketersediaan lintas permukaan

**Skills kustom tidak tersinkronisasi antar permukaan**. Skills yang diunggah ke satu permukaan tidak secara otomatis tersedia di permukaan lain:
...
Anda perlu mengelola dan mengunggah Skills secara terpisah untuk setiap permukaan di mana Anda ingin menggunakannya.
...
* **Claude API**:
  ...
  * **Tidak ada instalasi paket saat runtime**: Hanya paket yang sudah diinstal sebelumnya yang tersedia. Anda tidak dapat menginstal paket baru selama eksekusi.

---
source: platform
url: https://platform.claude.com/docs/id/resources/overview
fetched_at: 2026-07-25T03:07:29.726338Z
sha256: 11c4c212f41deb58afd71c62b325e332246c4c3dd87c12019dba7ef109c08ab7
---

# Sumber Daya

Model card, sumber daya pembelajaran, dan format dokumentasi yang dioptimalkan untuk konsumsi AI.

---

## Model card

<CardGroup cols={3}>
  <Card title="System Card Claude Opus 5" icon="file" href="https://www.anthropic.com/claude-opus-5-system-card">
    Dokumentasi terperinci tentang Claude Opus 5.
  </Card>

  <Card title="System Card Claude Sonnet 5" icon="file" href="https://www.anthropic.com/claude-sonnet-5-system-card">
    Dokumentasi terperinci tentang Claude Sonnet 5.
  </Card>

  <Card title="System Card Claude Fable 5 dan Mythos 5" icon="file" href="https://www.anthropic.com/claude-fable-5-mythos-5-system-card">
    Dokumentasi terperinci tentang Claude Fable 5 dan Claude Mythos 5.
  </Card>

  <Card title="System Card Claude Opus 4.8" icon="file" href="https://www.anthropic.com/claude-opus-4-8-system-card">
    Dokumentasi terperinci tentang Claude Opus 4.8.
  </Card>

  <Card title="System Card Claude Opus 4.7" icon="file" href="https://www.anthropic.com/claude-opus-4-7-system-card">
    Dokumentasi terperinci tentang Claude Opus 4.7.
  </Card>

  <Card title="System Card Claude Mythos Preview" icon="file" href="https://www.anthropic.com/claude-mythos-preview-system-card">
    Dokumentasi terperinci tentang Claude Mythos Preview.
  </Card>

  <Card title="System Card Claude Sonnet 4.6" icon="file" href="https://www.anthropic.com/claude-sonnet-4-6-system-card">
    Dokumentasi terperinci tentang Claude Sonnet 4.6.
  </Card>

  <Card title="System Card Claude Opus 4.6" icon="file" href="https://www.anthropic.com/claude-opus-4-6-system-card">
    Dokumentasi terperinci tentang Claude Opus 4.6.
  </Card>

  <Card title="System Card Claude Opus 4.5" icon="file" href="https://www.anthropic.com/claude-opus-4-5-system-card">
    Dokumentasi terperinci tentang Claude Opus 4.5.
  </Card>

  <Card title="System Card Claude Haiku 4.5" icon="file" href="https://www.anthropic.com/claude-haiku-4-5-system-card">
    Dokumentasi terperinci tentang Claude Haiku 4.5.
  </Card>

  <Card title="System Card Claude Sonnet 4.5" icon="file" href="https://www.anthropic.com/claude-sonnet-4-5-system-card">
    Dokumentasi terperinci tentang Claude Sonnet 4.5.
  </Card>

  <Card title="System Card Claude Opus 4.1" icon="file" href="https://www.anthropic.com/claude-opus-4-1-system-card">
    Dokumentasi terperinci tentang Claude Opus 4.1.
  </Card>

  <Card title="System Card Claude 4" icon="file" href="https://www.anthropic.com/claude-4-system-card">
    Dokumentasi terperinci tentang model Claude 4.
  </Card>

  <Card title="System Card Claude Sonnet 3.7" icon="file" href="https://www.anthropic.com/claude-3-7-sonnet-system-card">
    System card untuk Claude Sonnet 3.7 dengan detail kinerja dan keamanan.
  </Card>

  <Card title="System Card Claude Haiku 3.5 dan Sonnet 3.5" icon="file" href="https://www-cdn.anthropic.com/c7822cdc35ad788ec87e14b3a9d45010f1f86c38.pdf">
    Dokumentasi terperinci tentang Claude Haiku 3.5 dan Claude Sonnet 3.5.
  </Card>

  <Card title="Model Card Claude 3" icon="file" href="https://www.anthropic.com/claude-3-model-card">
    Dokumentasi terperinci tentang model Claude 3 termasuk adendum 3.5 terbaru.
  </Card>

  <Card title="Model Card Claude 2" icon="file" href="https://www.anthropic.com/claude-2-model-card">
    Dokumentasi terperinci tentang model Claude 2.
  </Card>
</CardGroup>

## Sumber daya pembelajaran

<CardGroup cols={3}>
  <Card title="Quickstart" icon="lightning" href="https://github.com/anthropics/anthropic-quickstarts">
    Aplikasi siap deploy yang dibangun dengan API.
  </Card>

  <Card title="Kursus" icon="graduation-cap" href="https://anthropic.skilljar.com/">
    Pelajaran langkah demi langkah tentang membangun dengan Claude.
  </Card>

  <Card title="Cookbook" icon="fork-knife" href="https://platform.claude.com/cookbooks">
    Contoh kode dan implementasi yang dapat direplikasi.
  </Card>

  <Card title="Panduan kasus penggunaan" icon="compass" href="/docs/id/about-claude/use-case-guides/overview">
    Panduan produksi mendalam untuk membangun kasus penggunaan umum dengan Claude.
  </Card>

  <Card title="Glosarium" icon="book-bookmark" href="/docs/id/about-claude/glossary">
    Istilah dan konsep kunci untuk bekerja dengan Claude dan model bahasa.
  </Card>
</CardGroup>

## Sumber daya untuk konsumsi AI

<CardGroup cols={3}>
  <Card title="Primer API untuk konsumsi Claude" icon="settings" href="/docs/id/claude_api_primer">
    Panduan API ringkas yang dimaksudkan untuk dikonsumsi oleh Claude.
  </Card>

  <Card title="Ikhtisar dokumentasi Claude Platform" icon="robot" href="/docs/for-claude">
    Ikhtisar ringkas dokumentasi Claude Platform, dioptimalkan untuk konsumsi LLM.
  </Card>

  <Card title="llms.txt" icon="file" href="/llms.txt">
    Indeks dokumentasi yang dioptimalkan untuk LLM.
  </Card>
</CardGroup>

...
sha256: 70257a96f570b0e5c1f8c948f3abb74d3a810cce38337ac3c77eeb274feceec4
---
...
<Note>
  Mendaftar melalui AWS Console akan menyediakan organisasi Anthropic baru yang terikat dengan akun AWS Anda. Organisasi ini terpisah dari organisasi yang sudah ada yang dimiliki perusahaan Anda dengan Anthropic, termasuk organisasi Claude Enterprise yang diperoleh melalui AWS Marketplace. Kunci API, workspace, dan pengaturan Claude Console dari organisasi Anthropic pihak pertama tidak akan terbawa.
...
<Steps>
  ...
  <Step title="Buat workspace Anda dan catat ID-nya">
    Setelah Anda menyelesaikan penyiapan, AWS Console meminta Anda untuk membuat workspace. Lihat [Workspaces](#workspaces) untuk detail tentang pengikatan region, pencakupan sumber daya IAM, dan pembuatan workspace tambahan.
  ...
  <Step title="Masuk ke Claude Console">
    ...
    3. Pada masuk pertama kali, Anda diminta memasukkan alamat email. Masukkan email kerja Anda. Platform menyediakan pengguna Claude Console Anda secara just-in-time.

    Saat Anda masuk melalui AWS Console, Claude Console mencakup organisasi Claude Platform di AWS Anda. Indikator **Account managed by AWS** muncul di kiri bawah sidebar Claude Console.
  </Step>
</Steps>
...
* **Tidak ada indikator kemajuan selama pendaftaran:** Pendaftaran memerlukan beberapa menit. Halaman menampilkan banner statis **Sign-up in progress** tanpa bilah kemajuan sementara AWS menyediakan akun Anda.
...
Region wajib diisi. Klien SDK akan memunculkan error jika tidak ada region yang diatur. Teruskan `aws_region`/`awsRegion` ke konstruktor, atau atur `AWS_REGION` (atau `AWS_DEFAULT_REGION`). Semua region komersial AWS didukung.
...
Untuk jalur integrasi yang lebih sederhana (pengembangan lokal dan skrip), Anda dapat melakukan autentikasi dengan kunci API alih-alih SigV4. Atur variabel lingkungan `ANTHROPIC_AWS_API_KEY` atau teruskan `apiKey` ke konstruktor SDK.
...
Teruskan token yang dihasilkan ke SDK dengan cara yang sama seperti Anda meneruskan kunci API yang dihasilkan AWS Console:
...
SDK tidak menyegarkan kunci jangka pendek secara otomatis. Ketika token kedaluwarsa, buat yang baru dan bangun klien baru. Principal yang menggunakan token tetap memerlukan tindakan IAM `aws-external-anthropic:CallWithBearerToken`.
...
<Note>
  Klien SDK untuk Claude Platform di AWS berada dalam tahap beta.
</Note>
...
Claude Platform di AWS menggunakan endpoint API yang sama dengan Claude API pihak pertama. Perbedaannya adalah base URL, metode autentikasi, dan header `anthropic-workspace-id` yang wajib ada yang mengidentifikasi [workspace](#workspaces) mana yang menjadi target permintaan.
...
* **Akses fitur:** Karena Anthropic mengoperasikan kedua platform, sebagian besar fitur baru dan header beta menjadi tersedia di Claude Platform di AWS tanpa langkah integrasi terpisah. Lihat [keterbatasan fitur](#features-not-supported) untuk pengecualian.
...
[Claude Managed Agents](/docs/id/managed-agents/overview) tersedia di Claude Platform di AWS, termasuk [agen](/docs/id/managed-agents/agent-setup), [lingkungan](/docs/id/managed-agents/environments), [sesi](/docs/id/managed-agents/sessions), [vault kredensial](/docs/id/managed-agents/vaults), [penyimpanan memori](/docs/id/managed-agents/memory), [webhook](/docs/id/managed-agents/webhooks), [orkestrasi multiagen](/docs/id/managed-agents/multi-agent), dan [sandbox yang dihosting sendiri](/docs/id/managed-agents/self-hosted-sandboxes).

Perilaku sesi di Claude Platform di AWS berbeda dari Claude Managed Agents pihak pertama dalam satu hal:

* **Reautentikasi sesi otonom:** Sebuah sesi dapat berjalan secara otonom, tanpa [event pengguna](/docs/id/managed-agents/reference#event-types) apa pun, hingga 6 jam. Setelah 6 jam, sesi memerlukan reautentikasi sebelum dapat dilanjutkan. Untuk melakukan reautentikasi, kirim event dengan peran pengguna apa pun ke sesi tersebut (lihat [Event dan streaming](/docs/id/managed-agents/events-and-streaming)). Claude Managed Agents pihak pertama tidak memiliki batas waktu berjalan sesi otonom.
...
Permintaan inferensi dan sumber daya di Claude Platform di AWS menargetkan sebuah workspace. Anda meneruskan ID workspace di header `anthropic-workspace-id` pada panggilan API ini. ID workspace menggunakan format bertag `wrkspc_` diikuti oleh pengidentifikasi alfanumerik (misalnya, `wrkspc_01AbCdEf23GhIj`). Lihat [Dapatkan ID workspace Anda](#obtain-your-workspace-id) jika Anda belum memilikinya.

### Pencakupan workspace
...
Buat workspace tambahan, ganti nama workspace, atau arsipkan workspace dari halaman **Workspaces** di AWS Console atau dengan endpoint workspace [Admin API](/docs/id/manage-claude/admin-api). Workspace baru terikat pada region AWS dari endpoint yang Anda panggil untuk membuatnya (lihat [Pencakupan workspace](#workspace-scoping)). Halaman Workspaces di Claude Console bersifat hanya-baca.

## Menggunakan Claude Console

Claude Platform di AWS menggunakan Claude Console standar di [platform.claude.com](https://platform.claude.com). Saat Anda masuk dari AWS Console, indikator **Account managed by AWS** muncul di kiri bawah sidebar Claude Console dan Console mencakup organisasi Claude Platform di AWS Anda. Console menyediakan analitik penggunaan, rincian biaya, visibilitas batas laju, visibilitas workspace, dan halaman untuk mengelola file, Agent Skills, pekerjaan batch, dan sumber daya Claude Managed Agents (agen, sesi, lingkungan, vault kredensial, penyimpanan memori, dan webhook).
...
4. Pada masuk pertama kali, Anda diminta memasukkan alamat email; masukkan email kerja Anda. Platform menyediakan pengguna Claude Console Anda secara just-in-time.
...
| **Cost**              | Ya       | Tidak               | Lihat rincian biaya berdasarkan model dan workspace. AWS Cost Explorer menampilkan item baris [Claude Consumption Unit (CCU)](#billing) yang diagregasi. |
| **Limits**            | Ya       | Tidak               | Lihat batas laju (hanya-baca).                                                                                                                           |
...
| **Billing**           | Tidak    | N/A                 | Tidak berlaku. AWS Marketplace mengelola penagihan dan faktur. Lihat rincian biaya di halaman Cost.                                                      |
...
Organisasi di Claude Platform on AWS tidak berpindah antar tier penggunaan secara otomatis. Untuk meminta batas yang lebih tinggi, hubungi perwakilan akun Anthropic Anda. Untuk detail tier dan batas per-model, lihat [Batas laju](/docs/id/api/rate-limits).

## Penagihan

Claude Platform on AWS menagih melalui [AWS Marketplace](https://aws.amazon.com/marketplace). Penggunaan didenominasikan dalam Claude Consumption Units (CCU), diukur per jam, dan ditagihkan bulanan di belakang pada tagihan AWS Anda. CCU bukan kredit prabayar; tidak ada saldo atau komitmen CCU.
...
AWS CloudTrail dapat menangkap semua permintaan ke Claude Platform on AWS. Operasi workspace, vault, dan webhook dicatat sebagai event Management secara default. Operasi inferensi, batch, file, skill, model, profil pengguna, dan Claude Managed Agents (selain vault dan webhook) diklasifikasikan sebagai event Data dan memerlukan konfigurasi pencatatan data event secara eksplisit, yang menimbulkan biaya CloudTrail tambahan. Lihat [referensi tindakan IAM](/docs/id/api/claude-platform-on-aws-iam-actions#route-to-action-mapping) untuk klasifikasi tipe event lengkap dan [dokumentasi AWS CloudTrail](https://docs.aws.amazon.com/awscloudtrail/latest/userguide/) untuk detail konfigurasi.

### ID permintaan

Setiap respons menyertakan dua ID permintaan di header respons:

* **AWS request ID (`x-amzn-requestid`):** ID utama, diindeks di CloudTrail. Gunakan ini saat menyelidiki permintaan melalui perkakas AWS atau saat menghubungi dukungan AWS.
...
Anthropic merekomendasikan pencatatan aktivitas Anda setidaknya dengan basis bergulir 30 hari untuk memahami pola penggunaan dan menyelidiki potensi masalah apa pun.
...
| **ID model**            | `anthropic.claude-haiku-4-5`                                                                             | `anthropic.claude-haiku-4-5-20251001-v1:0` (dengan prefiks opsional `us.`/`global.`)                     | `claude-haiku-4-5`                                                                           |
...
<Warning>
  **Aktifkan outbound web identity federation terlebih dahulu.** Jika akun AWS Anda belum pernah menggunakan Claude Platform on AWS sebelumnya, Anda harus [mengaktifkan outbound web identity federation](#enable-outbound-web-identity-federation) satu kali per akun sebelum membuat permintaan. Tanpa langkah ini, semua permintaan gagal dengan error federasi (lihat [Aktifkan outbound web identity federation](#enable-outbound-web-identity-federation) untuk error yang tepat dan perbaikannya). Langkah ini tidak diperlukan untuk Bedrock.
</Warning>
...
Tindakan `GetBatchInference` mengotorisasi baik rute metadata batch maupun rute hasil batch. Menolaknya memblokir kedua pembacaan tersebut. Untuk kebijakan Deny-only yang cocok untuk beban kerja yang sensitif terhadap ZDR, lihat [Penguncian fitur untuk workspace yang sensitif terhadap ZDR](/docs/id/api/claude-platform-on-aws-iam-actions#feature-lockdown-for-a-zdr-sensitive-workspace).

<Note>
  `ListWorkspaces` berlingkup akun, sehingga muncul dalam pernyataan Allow terpisah dengan `"Resource": "*"`. Menentukan ARN workspace pada tindakan berlingkup akun tidak memiliki efek (lihat [Otomatisasi provisioning](/docs/id/api/claude-platform-on-aws-iam-actions#provisioning-automation)).

  Kebijakan ini mengasumsikan autentikasi AWS SigV4. Jika principal melakukan autentikasi dengan kunci API, tambahkan juga `aws-external-anthropic:CallWithBearerToken` ke pernyataan Allow `"Resource": "*"`. `CallWithBearerToken` adalah tindakan lapisan autentikasi tanpa rute yang tidak terikat pada ARN workspace. Lihat [Isolasi workspace per-pelanggan](/docs/id/api/claude-platform-on-aws-iam-actions#per-customer-workspace-isolation) untuk pola dua pernyataan.
</Note>

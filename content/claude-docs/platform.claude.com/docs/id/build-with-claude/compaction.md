...
sha256: b161dcfad9b08c31adeb11476f8c53d3eb13030ea3264e1fcbf49bd923c71e6d
---
...
![Alur compaction (pemadatan): ketika input tokens (token input) mencapai trigger (pemicu), Claude menulis ringkasan ke dalam compaction block (blok pemadatan) dan melanjutkan](https://platform.claude.com/docs/images/compaction-flow.svg)
...
Compaction memerlukan langkah sampling tambahan, yang berkontribusi pada "rate limit" (batas laju) dan penagihan. API mengembalikan informasi penggunaan terperinci dalam respons:
...
Endpoint penghitungan token (`/v1/messages/count_tokens`) menerapkan blok `compaction` yang ada dalam prompt Anda tetapi tidak memicu compaction baru. Gunakan endpoint ini untuk memeriksa jumlah token efektif Anda setelah compaction sebelumnya:
...
<CodeGroup>
  ...
  var response = await client.Beta.Messages.Create(new MessageCreateParams
  {
      ...
      // max_tokens membatasi seluruh panggilan, termasuk proses pemikiran apa pun, jadi sediakan beberapa ribu token.
      MaxTokens = 4096,

...
sha256: b9dbd23071e81d4172fc36ec205a498fb770692c4130afce78e350bcbc7fee95
---
...
"Adaptive thinking" (pemikiran adaptif) adalah cara yang direkomendasikan untuk menggunakan [pemikiran diperpanjang](/docs/id/build-with-claude/extended-thinking) dengan Claude Opus 4.8, Claude Opus 4.7, Claude Opus 4.6, Claude Sonnet 5, dan Claude Sonnet 4.6, dan merupakan satu-satunya mode pemikiran pada Claude Fable 5 dan [Claude Mythos 5](https://anthropic.com/glasswing). Alih-alih mengatur anggaran token pemikiran secara manual, pemikiran adaptif memungkinkan Claude secara dinamis menentukan kapan dan seberapa banyak menggunakan pemikiran diperpanjang berdasarkan kompleksitas setiap permintaan. Default dan pembatasan per model tercantum di bawah [Model yang didukung](#supported-models).

<Tip>
  Pemikiran adaptif dapat menghasilkan kinerja yang lebih baik daripada pemikiran diperpanjang dengan `budget_tokens` tetap untuk banyak beban kerja, terutama beban kerja yang mencampur permintaan sepele dan kompleks, serta alur kerja agentik jangka panjang. Tidak diperlukan header beta.

  Jika beban kerja Anda memerlukan latensi yang dapat diprediksi atau kontrol presisi atas biaya pemikiran, pemikiran diperpanjang dengan `budget_tokens` masih berfungsi pada Claude Opus 4.6 dan Claude Sonnet 4.6 tetapi sudah usang (deprecated) dan tidak lagi direkomendasikan. Lihat peringatan penghentian di [Model yang didukung](#supported-models).
</Tip>
...
* Claude Fable 5 (claude-fable-5) dan Claude Mythos 5 (claude-mythos-5), pemikiran adaptif selalu aktif; `thinking: {type: "disabled"}` tidak didukung. Kedua model tidak tersedia di bawah [zero data retention](/docs/id/manage-claude/api-and-data-retention#model-specific-data-retention-requirements).
* Claude Mythos Preview (claude-mythos-preview), pemikiran adaptif adalah default; `thinking: {type: "disabled"}` tidak didukung, dan `{type: "enabled", budget_tokens: N}` manual masih diterima.
* Claude Opus 4.8 (claude-opus-4-8), pemikiran adaptif adalah satu-satunya mode pemikiran yang didukung. Pemikiran nonaktif kecuali Anda secara eksplisit mengatur `thinking: {type: "adaptive"}` dalam permintaan Anda; `thinking: {type: "enabled"}` manual ditolak dengan error 400.
* Claude Opus 4.7 (claude-opus-4-7), pemikiran adaptif adalah satu-satunya mode pemikiran yang didukung. Pemikiran nonaktif kecuali Anda secara eksplisit mengatur `thinking: {type: "adaptive"}` dalam permintaan Anda; `thinking: {type: "enabled"}` manual ditolak dengan error 400.
* Claude Opus 4.6 (claude-opus-4-6), pemikiran adaptif nonaktif kecuali Anda secara eksplisit mengatur `thinking: {type: "adaptive"}`; `{type: "enabled", budget_tokens: N}` manual masih diterima tetapi sudah usang.
* Claude Sonnet 5 (claude-sonnet-5), pemikiran adaptif aktif secara default; berikan `thinking: {type: "disabled"}` untuk menonaktifkannya. `{type: "enabled"}` manual ditolak dengan error 400.
* Claude Sonnet 4.6 (claude-sonnet-4-6), pemikiran adaptif nonaktif kecuali Anda secara eksplisit mengatur `thinking: {type: "adaptive"}`; `{type: "enabled", budget_tokens: N}` manual masih diterima tetapi sudah usang.

<Warning>
  `thinking.type: "enabled"` dan `budget_tokens` sudah [**usang**](/docs/id/build-with-claude/overview#feature-availability) pada Opus 4.6 dan Sonnet 4.6 dan akan dihapus pada rilis model mendatang. Gunakan `thinking.type: "adaptive"` dengan parameter [effort](/docs/id/build-with-claude/effort) sebagai gantinya. Konfigurasi `budget_tokens` yang ada masih berfungsi tetapi tidak lagi direkomendasikan; rencanakan untuk bermigrasi.

  Model yang lebih lama, seperti Claude Sonnet 4.5 dan Claude Opus 4.5, tidak mendukung pemikiran adaptif dan memerlukan `thinking.type: "enabled"` dengan `budget_tokens`.
</Warning>

## Cara kerja pemikiran adaptif

Dalam mode adaptif, pemikiran bersifat opsional bagi model. Claude mengevaluasi kompleksitas setiap permintaan dan menentukan apakah dan seberapa banyak menggunakan pemikiran diperpanjang. Pada tingkat [effort](/docs/id/build-with-claude/effort) default (`high`), Claude hampir selalu berpikir. Pada tingkat effort yang lebih rendah, Claude dapat melewati pemikiran untuk masalah yang lebih sederhana.

Pemikiran adaptif juga secara otomatis mengaktifkan [interleaved thinking](/docs/id/build-with-claude/extended-thinking#interleaved-thinking). Ini berarti Claude dapat berpikir di antara pemanggilan alat, menjadikannya sangat efektif untuk alur kerja agentik.

## Cara menggunakan pemikiran adaptif

Atur `thinking.type` ke `"adaptive"` dalam permintaan API Anda. Contoh-contoh ini juga mengatur `thinking.display` ke `"summarized"` untuk membuat teks pemikiran terlihat: pada model terbaru, `display` secara default adalah `"omitted"`, yang mengembalikan blok pemikiran dengan field `thinking` kosong. Lihat [Mengontrol tampilan pemikiran](#controlling-thinking-display) untuk detailnya.
...
Token pemikiran dihitung terhadap `max_tokens`, jadi atur cukup tinggi untuk menyisakan ruang bagi pemikiran dan teks respons. Lihat [Kontrol biaya](#cost-control).
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `max`            | Claude selalu berpikir tanpa batasan pada kedalaman pemikiran. Tersedia pada semua model yang mendukung pemikiran adaptif.                                                     |
| `xhigh`          | Claude selalu berpikir secara mendalam dengan eksplorasi yang diperluas. Tersedia pada Claude Fable 5, Claude Mythos 5, Claude Opus 4.8, Claude Opus 4.7, dan Claude Sonnet 5. |
| `high` (default) | Claude hampir selalu berpikir. Memberikan penalaran mendalam pada tugas-tugas kompleks.                                                                                        |
| `medium`         | Claude menggunakan pemikiran moderat. Dapat melewati pemikiran untuk kueri sederhana.                                                                                          |
...
Pemikiran adaptif bekerja dengan [streaming](/docs/id/build-with-claude/streaming). Blok pemikiran di-stream melalui event `thinking_delta`, sama seperti dalam mode pemikiran manual. Seperti pada contoh sebelumnya, `thinking.display: "summarized"` membuat teks pemikiran yang di-stream terlihat:
...
| ----------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| **Adaptif**       | `thinking: {type: "adaptive"}`                  | Claude Fable 5 (selalu aktif), Claude Mythos 5 (selalu aktif), Claude Mythos Preview (default), Claude Opus 4.8 (satu-satunya mode), Claude Opus 4.7 (satu-satunya mode), Claude Opus 4.6, Claude Sonnet 5 (default), dan Claude Sonnet 4.6 | Claude menentukan kapan dan seberapa banyak menggunakan pemikiran diperpanjang. Gunakan `effort` untuk memandu. |
| **Manual**        | `thinking: {type: "enabled", budget_tokens: N}` | Semua model kecuali Claude Fable 5, Claude Mythos 5, Claude Sonnet 5, Claude Opus 4.8, dan Claude Opus 4.7 (ditolak dengan error 400). Usang pada Opus 4.6 dan Sonnet 4.6 (pertimbangkan mode adaptif sebagai gantinya).                    | Ketika Anda memerlukan kontrol presisi atas pengeluaran token pemikiran.                                        |
| **Dinonaktifkan** | `thinking: {type: "disabled"}`                  | Semua model kecuali Claude Fable 5, Claude Mythos 5, dan Claude Mythos Preview. Pada Claude Sonnet 5, berikan `{type: "disabled"}` secara eksplisit (menghilangkan `thinking` secara default menjadi adaptif).                              | Ketika Anda tidak memerlukan pemikiran diperpanjang dan menginginkan latensi terendah.                          |

<Note>
  Default dan pembatasan per model tercantum di bawah [Model yang didukung](#supported-models). Model yang lebih lama dari yang tercantum hanya menerima `type: "enabled"` dengan `budget_tokens`, jika mereka mendukung pemikiran diperpanjang sama sekali.

  **Ketersediaan interleaved thinking berdasarkan mode:**

  * **Mode adaptif:** Interleaved thinking secara otomatis diaktifkan pada Claude Fable 5, Claude Mythos 5, Claude Mythos Preview, Claude Opus 4.8, Claude Opus 4.7, Opus 4.6, Sonnet 5, dan Sonnet 4.6. Pada Claude Fable 5, Claude Mythos 5, Mythos Preview, Claude Opus 4.8, dan Opus 4.7, penalaran antar-alat selalu berada di dalam blok pemikiran.
...
Secara terpisah, Claude Fable 5, Claude Mythos 5, Claude Mythos Preview, Claude Opus 4.8, Claude Opus 4.7, dan Claude Sonnet 5 menolak nilai `temperature`, `top_p`, dan `top_k` non-default dengan error 400. Ini berlaku untuk setiap permintaan pada model-model ini, terlepas dari apakah pemikiran aktif.

### Caching prompt

Permintaan berturut-turut yang menggunakan pemikiran `adaptive` mempertahankan breakpoint [prompt cache](/docs/id/build-with-claude/prompt-caching). Namun, beralih antara mode pemikiran `adaptive` dan `enabled`/`disabled` merusak breakpoint cache untuk pesan. Prompt sistem dan definisi alat tetap di-cache terlepas dari perubahan mode.
...
Efektivitas pengarahan dapat sensitif terhadap kata-kata yang tepat. Jika satu frasa tidak menghasilkan perilaku yang Anda inginkan, coba varian yang lebih langsung.
...
<Warning>
  Mengarahkan Claude untuk berpikir lebih jarang dapat mengurangi kualitas pada tugas-tugas yang mendapat manfaat dari penalaran. Ukur dampaknya pada beban kerja spesifik Anda sebelum menerapkan penyetelan berbasis prompt ke produksi. Pertimbangkan untuk menguji dengan [tingkat effort](/docs/id/build-with-claude/effort) yang lebih rendah terlebih dahulu.
</Warning>

### Kontrol biaya

Gunakan `max_tokens` sebagai batas keras pada total output (pemikiran + teks respons). Parameter `effort` memberikan panduan lunak tambahan tentang seberapa banyak pemikiran yang dialokasikan Claude. Bersama-sama, keduanya memberi Anda kontrol yang efektif atas biaya.

Pada tingkat effort `high` dan `max`, Claude dapat berpikir lebih ekstensif dan lebih mungkin menghabiskan anggaran `max_tokens`. Jika Anda mengamati `stop_reason: "max_tokens"` dalam respons, pertimbangkan untuk meningkatkan `max_tokens` untuk memberi model lebih banyak ruang, atau menurunkan tingkat effort.
...
<Note>
  ...
  * **Claude Opus 4.6 dan Claude Sonnet 4.6:** default-nya adalah `"summarized"`. Ringkasan yang dapat dibaca muncul tanpa perlu memilih untuk mengaktifkannya.

  Untuk menerima teks pemikiran yang diringkas pada model di mana default-nya adalah `"omitted"`, atur `thinking.display` ke `"summarized"` secara eksplisit:
...
Dua pengecualian, dibahas di [Fallback credit](/docs/id/build-with-claude/fallback-credit):

* Percobaan ulang fallback-credit harus menggemakan body permintaan yang ditolak tanpa perubahan.
* Blok `fallback` dari fallback di tengah output tetap berada di tempat mereka muncul.

Untuk mendapatkan visibilitas ke dalam penalaran model, baca blok `thinking` yang dijelaskan di bagian ini alih-alih meminta penalaran dalam teks respons. Pada Claude Fable 5, permintaan yang mencoba memancing penalaran internal model sebagai bagian dari teks respons dapat ditolak dengan `stop_details.category: "reasoning_extraction"`. Lihat [Kategori penolakan](/docs/id/build-with-claude/refusals-and-fallback#refusal-response) untuk referensi field dan panduan penanganan.
...
Halaman pemikiran diperpanjang membahas beberapa topik secara lebih rinci dengan contoh kode spesifik per mode:
...
* **[Pemikiran diperpanjang dengan caching prompt](/docs/id/build-with-claude/extended-thinking#extended-thinking-with-prompt-caching):** Contoh kode untuk caching dengan blok pemikiran. Perilaku cache khusus adaptif dibahas di [Caching prompt](#prompt-caching) sebelumnya di halaman ini.
...
<CardGroup cols={3}>
  ...
  <Card title="Pemikiran diperpanjang" icon="settings" href="/docs/id/build-with-claude/extended-thinking">
    Berikan Claude penalaran yang ditingkatkan untuk tugas-tugas kompleks dan kontrol bagaimana konten pemikiran dikembalikan.
  </Card>

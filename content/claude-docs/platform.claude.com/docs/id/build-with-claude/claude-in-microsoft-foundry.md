...
Respons API dari Claude di Foundry mengikuti [format respons API Claude](/docs/id/api/messages) standar. Ini termasuk objek `usage` dalam badan respons, yang memberikan informasi konsumsi token terperinci untuk permintaan Anda. Objek `usage` konsisten di semua platform (API pihak pertama, Foundry, Amazon Bedrock, dan Google Vertex AI).
...
Anthropic merekomendasikan logging aktivitas Anda setidaknya pada dasar rolling 30 hari untuk memahami pola penggunaan dan menyelidiki potensi masalah.

...
sha256: 877d583c81af5e97a2f7c797f16db866621fefb45d4a702763f14f942fdd4239
---

---
title: Rancang integrasi kepatuhan Anda
url: https://platform.claude.com/docs/id/manage-claude/compliance-integration-patterns
description: Pilih antara polling dan konsumsi Activity Feed berbasis kursor, korelasikan peristiwa Compliance API dengan SIEM Anda, dan rencanakan retensi.
---
...
Halaman ini mengasumsikan Anda telah membaca [Mengkueri Activity Feed](https://platform.claude.com/docs/id/manage-claude/compliance-activity-feed), yang mendefinisikan parameter dan kontrak paginasi yang dirujuk di seluruh halaman ini, dan [Mengambil dan menghapus obrolan, file, proyek, dan sesi](https://platform.claude.com/docs/id/manage-claude/compliance-content-data), yang mendefinisikan endpoint konten dan semantik `deleted_at` yang dirujuk di [Rencanakan retensi konten](https://platform.claude.com/docs/id/manage-claude/compliance-integration-patterns#plan-content-retention).
...
| Pembacaan inkremental berbasis kursor | Anda menginginkan latensi terendah antara terjadinya aktivitas dan pipeline Anda menyerapnya, Anda ingin menghindari pembacaan ulang halaman yang sudah Anda kosongkan, dan Anda memiliki tempat yang tahan lama untuk menyimpan kursor di antara eksekusi |
...
Ketika respons memiliki `has_more: true`, jendela tersebut berisi lebih dari satu halaman aktivitas. Lakukan paginasi di dalam jendela dengan meneruskan `last_id` dari respons sebagai `after_id` pada permintaan berikutnya (berhenti ketika `has_more` bernilai `false`), atau pilih jendela waktu yang lebih kecil. Lihat [Melakukan paginasi hasil](https://platform.claude.com/docs/id/manage-claude/compliance-activity-feed#paginate-results) untuk kontrak lengkapnya.
...
Lakukan paginasi hingga `has_more` bernilai `false`, lalu simpan `first_id` dari respons terakhir dan teruskan tanpa perubahan sebagai `before_id` pada eksekusi berikutnya untuk mengambil aktivitas yang lebih baru dari kursor yang disimpan. Untuk berjalan ke arah sebaliknya untuk backfill, simpan `last_id` dan teruskan sebagai `after_id`. Untuk referensi lengkap kursor-vs-page-token dan semantik retry, lihat [Melakukan paginasi hasil](https://platform.claude.com/docs/id/manage-claude/compliance-activity-feed#paginate-results).
...
Kursor tetap bertahan melewati rotasi kunci; lihat [Mengelola dan merotasi kunci](https://platform.claude.com/docs/id/manage-claude/compliance-api-access#manage-and-rotate-keys).

<Warning>
  Setiap halaman berdekatan dengan kursor yang Anda teruskan: loop berjalan maju menuju waktu sekarang, satu halaman pada satu waktu. Jangan memperlakukan satu respons sebagai sudah tersinkronisasi selama `has_more` bernilai `true`. Simpan kursor hanya setelah `has_more` bernilai `false`; halaman yang belum diambil adalah halaman yang lebih baru antara `first_id` respons ini dan waktu sekarang, dan halaman tersebut tetap belum terbaca sampai Anda menyelesaikan loop atau menjalankannya lagi.
</Warning>
...
Panggilan ke Compliance API itu sendiri menghasilkan aktivitas `compliance_api_accessed`. Serap aktivitas ini bersama dengan jenis aktivitas lainnya sehingga SIEM Anda mencatat siapa yang mengkueri data kepatuhan, dan kapan. Teruskan `activity_types[]=compliance_api_accessed` untuk membatasi cakupan kueri, lalu di klien Anda, baca `actor.api_key_id` dari setiap aktivitas yang `actor.type`-nya adalah `api_actor` untuk mengatribusikan akses ke Compliance Access Key atau kunci Admin API tertentu.
...
| Transkrip sesi remote (Cowork di claude.ai web dan mobile)      | 6 tahun                                                                                                            | Anthropic                                                                  |
| Transkrip sesi lokal (Cowork dan Claude Code di mesin pengguna) | 6 tahun secara default, atau periode retensi percakapan kustom organisasi Anda, ketika periode terbatas ditetapkan | Anthropic secara default; organisasi Anda ketika menetapkan periode kustom |
...
Untuk bagaimana bagian lain dari Claude Platform menangani retensi, lihat [Retensi API dan data](https://platform.claude.com/docs/id/manage-claude/api-and-data-retention).
...
Endpoint konten (obrolan, file, proyek, lampiran proyek, transkrip sesi remote Cowork, serta transkrip sesi lokal Cowork dan Claude Code) hanya menyajikan data Claude Enterprise; Activity Feed menampilkan peristiwa administratif dan sumber daya di seluruh organisasi. Compliance API tidak mencakup:
...
* Sesi lokal dari organisasi dengan [kesiapan HIPAA](https://platform.claude.com/docs/id/manage-claude/api-and-data-retention#hipaa-readiness) diaktifkan, dan sesi lokal yang [zero data retention](https://platform.claude.com/docs/id/manage-claude/api-and-data-retention#zero-data-retention-zdr-scope)-nya berlaku.
...
* Definisi alat dan konfigurasi server MCP dalam transkrip sesi (lokal atau remote), dan metadata sitasi pada blok `text` dalam transkrip sesi lokal.
* Konten transkrip untuk sesi lokal di organisasi yang menggunakan [kunci enkripsi yang dikelola pelanggan](https://platform.claude.com/docs/id/manage-claude/cmek) (metadata sesi tetap tercantum).
...
Untuk chain of custody, simpan catatan yang diekspor dengan metadata asal-usul: endpoint sumber, parameter kueri, timestamp eksekusi, dan hash konten dari setiap catatan.
